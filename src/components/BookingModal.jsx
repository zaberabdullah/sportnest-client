  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <form onSubmit={handleBooking} className="bg-zinc-900 p-6 rounded-2xl border border-zinc-700 w-full max-w-sm">
        ...
        <input name="slot" type="text" placeholder="e.g., 10:00-11:00" className="w-full p-2 mb-4 bg-zinc-800 rounded text-white" required />
        
        <div className="flex gap-2">
          <button type="button" onClick={onClose} className="flex-1 p-2 bg-zinc-700 text-white rounded">Cancel</button>
          <button type="submit" className="flex-1 p-2 bg-emerald-600 text-white rounded font-bold">Confirm</button>
        </div>
      </form>
    </div>
  );
